...
/**
 * Dual-browser collaboration smoke (v1.1.3 F4).
 *
...
test.describe('Collaboration M1 smoke (2 browsers)', () => {
  test('host creates room; viewer joins read-only', async ({ browser }) => {
    ...
    try {
      ...
      await expect(viewerPage.locator('.modal--collab .collab-role-badge')).toContainText('只读')
      await expect(hostPage.locator('.modal--collab .collab-role-badge')).toContainText('主持人')
    } finally {
