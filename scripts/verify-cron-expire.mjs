#!/usr/bin/env node
/**
 * Verify /api/billing/expire-subscriptions cron auth (local or production).
 *
 * Usage:
 *   CRON_SECRET=xxx node scripts/verify-cron-expire.mjs
 *   APP_URL=https://ai-ide-flame.vercel.app BILLING_CRON_SECRET=xxx node scripts/verify-cron-expire.mjs
 */
...
const url = `${base}/api/billing/expire-subscriptions`
...
if (okAuth !== 200) {
  console.error('Expected 200 with valid secret (check APP_URL and database)')
  process.exit(1)
}

console.log('✅ Cron expire endpoint auth OK')
