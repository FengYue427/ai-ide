...
export const JA_JP_BULK_OVERRIDES: Partial<Record<TranslationKey, string>> = {
  ...
  'settings.ai.usageQuotaNearLimit': '本日のクォータ {percent}%（{used}/{limit}）— 上限に近い',
  'settings.ai.usagePlatformProvider': 'プラットフォームルート',
  'settings.pluginOps.desc': 'GET /api/health のサーバー状態。レビューキューはインスタンスメモリのみ。',
  'settings.pluginOps.envDocLink': 'Vercel 本番環境変数',
  'settings.pluginOps.clientFlagsLink': 'v1.2.6 クライアントフラグ',
  'settings.pluginOps.reviewsTitle': '自分の公開レビュー',
  'settings.pluginOps.reviewsEmpty': '提出なし（PLUGIN_PUBLISH_ENABLED が必要）',
  'settings.pluginOps.reviewsRefresh': 'レビュー一覧を更新',
  'settings.pluginOps.reviewsHint': 'reviewId はログ照合用。コールドスタートでサーバー一覧は消えます。',
  'settings.pluginOps.reviewsFilter': 'ステータス絞り込み',
  'settings.pluginOps.reviewsFilterAll': 'すべて',
  'settings.pluginOps.reviewsFilterPending': '審査待ち',
  'settings.pluginOps.reviewsCount': '合計 {total} 件 · 審査待ち {pending}',
  'settings.v13.title': 'v1.3 機能（読み取り専用）',
  'settings.v13.desc': 'Python ナビ、embedding 永続キャッシュ、インデックス構築テレメトリ、バックグラウンド Agent の状態。',
  'settings.v13.pythonNav': 'Python ファイル間ナビ',
  'settings.v13.embeddingPersist': 'Embedding 永続キャッシュ',
  'settings.v13.indexTelemetry': 'インデックス構築テレメトリ',
  'settings.v13.backgroundAgent': 'バックグラウンド Agent UI',
  'settings.v13.embeddingMetrics': 'Embedding キャッシュ: ヒット {hits} · ミス {misses} · 期限切れ {expired}',
  'settings.v13.indexBuildMode': '前回インデックス: {mode} · {ms}ms',
  'settings.v14.title': 'v1.4 機能（読み取り専用）',
  'settings.v14.desc': 'v1.4 本番ポリシー: Tab/FIM · 2k index · Git hunk · desktop · background Agent · MCP/plugins.',
  'settings.v14.tabFimProduction': 'Tab/FIM 本番ポリシー',
  'settings.v14.p95Target': 'Tab 遅延目標: P95 < {ms}ms',
  'settings.v14.productionDebounce': '本番既定 debounce: {ms}ms',
  ...
  'settings.tabCompletion.metricsCardTitle': 'Tab 補完メトリクス（v1.3）',
  'settings.backgroundAgent.title': 'バックグラウンド Agent',
  'settings.backgroundAgent.desc': 'タブを閉じた後もサーバーで Agent を継続。VITE_BACKGROUND_AGENT と Cron が必要。',
  'settings.backgroundAgent.enabled': 'クライアント UI',
  'settings.backgroundAgent.cron': 'ジョブ処理',
  'settings.backgroundAgent.envHint': '本番では既定オフ。docs/V1.1_FEATURE_FLAGS.md と V1.3_ENV.md を参照。',
  'settings.index.telemetry': '前回のインデックス {ms}ms（{mode}）',
  ...
  'settings.tabCompletion.desc':
    'Inline Tab completions in the editor. BYOK, platform AI, and Ollama supported; FIM preferred for DeepSeek / Qwen / Ollama.',
  ...
  'settings.tabCompletion.metricsDesc':
    'Hits {hits} · misses {misses} · skipped {skipped} · FIM ok {fim} (attempts {fimAttempts} · chat fallback {fimFallback}) · platform {platform} · chat {chat} · avg {avgMs} ms · last {last}',
  'settings.tabCompletion.metricsP95':
    'Latency percentiles: P50 {p50} ms · P95 {p95} ms · samples {samples} · target < {target} ms',
  'settings.tabCompletion.metricsP95Unknown':
    'Latency percentiles: no samples yet (target < {target} ms)',
  ...
  'chat.payload.meterLabel': 'コンテキスト 約 {estimatedKb}/{budgetKb} KB（{percent}%）',
  'chat.payload.meterAria': '予想ペイロード {estimatedKb} KB、予算 {budgetKb} KB、{percent}%',
  'chat.mention.preflightBannerTitle': '@ 言及プリフライト',
  'chat.mention.preflightUnresolved': '解決できません: {tokens}',
  'chat.mention.preflightTooMany': '@ は {max} 件まで（現在 {count}）',
  'chat.mention.preflightAmbiguous': 'シンボルが重複: {token}（{paths}）',
  'chat.mention.preflightSendHint': '送信時、未解決の @ はスキップされます',
  ...
  'chat.payload.meterMcpNote': 'MCP ツール予約を含む',
  'chat.mention.slimPastAmbiguous': 'スリム送信',
  'editor.references.peekTitle': '参照: {symbol}（{count}）',
  'editor.references.peekClose': '参照リストを閉じる',
  'editor.references.lineCol': '{line}:{column}',
  'plugin.publish.title': 'レビュー提出',
  'plugin.publish.desc': 'manifest + source をサーバー審査キューへ（要ログイン・PLUGIN_PUBLISH_ENABLED）',
  'plugin.publish.submit': 'レビュー提出',
  'plugin.publish.success': '提出済み — {reviewId}',
  'plugin.publish.failed': '提出失敗',
  'plugin.publish.loginRequired': '提出にはログインが必要です',
