# 1.0.4.x 附属路线图（1.0.4.1 → 1.0.4.4）— 规划

> **前置**：主版本 **[v1.0.4 GA](V1.0.4_MASTER_PLAN.md)**（**Kickoff 中**）  
> **短规划全文**：[PLAN_SHORT_V1.0.3-V1.0.4.md](./PLAN_SHORT_V1.0.3-V1.0.4.md) §4  
> **版本策略**：[VERSIONING.md](./VERSIONING.md)

---

## 四级总览

| 版本 | 代号 | 状态 | 主题 |
|:----:|------|:----:|------|
| **1.0.4** | — | 🔶 **RC** | 体验巩固主版本 · **1.0.4-rc.1** |
| **1.0.4.1** | **E1 MCP** | 🔶 | 官方 MCP 预置 · [V1.0.4.1_EXECUTION.md](./V1.0.4.1_EXECUTION.md) |
| **1.0.4.2** | **E2 感知** | 🔶 | 焦点/终端上下文 · [V1.0.4.2_EXECUTION.md](./V1.0.4.2_EXECUTION.md) |
| **1.0.4.3** | **E3 检索** | ⏳ | 索引重试、语义引导、i18n 补全 |
| **1.0.4.4** | **E4 收官** | ⏳ | 发布矩阵 + live 复测 · **~2.80** |

**合并部署建议**：`1.0.4.1+1.0.4.2` → tag `v1.0.4.2`；`1.0.4.3+1.0.4.4` → tag `v1.0.4.4`。

---

## 非目标

Background Agent · VSIX · DAP · Tab++ · Kiro Hooks — 见 [ROADMAP_V1.1.md](./ROADMAP_V1.1.md)。

---

## 文档

- [V1.0.4_MASTER_PLAN.md](./V1.0.4_MASTER_PLAN.md) · [V1.0.4_KICKOFF.md](./V1.0.4_KICKOFF.md)
- [V1.0.4.1_EXECUTION.md](./V1.0.4.1_EXECUTION.md) · [MCP_OFFICIAL_CATALOG.md](./MCP_OFFICIAL_CATALOG.md)
- `V1.0.4.2`～`V1.0.4.4` 执行清单（GA 后按需）
