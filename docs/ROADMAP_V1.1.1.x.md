...
- **状态**：✅（`package.json` **1.1.1**、tag `v1.1.1` 已推送）

---

## v1.1.1.10 — 计划总览快捷导航（GA 后）

- **用户故事**：在计划总览卡片一键打开最新报告，或滚动到计划/报告目录，无需在设置里翻找。
- **实现**：`PlanOverviewSection` 快捷按钮；`SettingsCenter` 锚点滚动；`PanelHost` 打开最新报告。
- **验收**：有报告时「打开最新报告」在编辑器打开；「查看计划/报告」平滑滚动到对应区块。

## v1.1.1.11 — 队列统计持久化 + 批量入队（GA 后）

- **用户故事**：刷新后仍能看到本会话队列成功/失败统计；多步计划可一键入队全部未完成步骤。
- **实现**：`queueSessionStatsPersistenceService`；`PlansSection`「入队全部未完成」。
- **验收**：刷新后统计与最近完成仍在；损坏数据 toast + 重置；3+ 步计划可一键入队。

---

## v1.1.1.12 — 创建/复制后同步提示（GA 后）

- **用户故事**：从模板创建或复制计划后，提示是否将 `.aide/` 同步到 AI 工作区索引。
- **实现**：`planAideSyncPromptService`；`PanelHost` 在创建/复制成功后 `offerSyncAideAfterPlanWrite`。
- **验收**：创建 plan 后弹出确认；点「同步」后 @ 可见；取消则不改变索引。

## v1.1.1.13 — PanelHost 计划/报告 i18n（GA 后）

- **用户故事**：切换 en-US 后，计划执行确认、映射、删除、报告清理等 toast/确认框为英文。
- **实现**：`plan.host.*` / `report.host.*` / `spec.host.*`；`PanelHost` 全部改用 `t()`。
- **验收**：en-US 下走完计划删除/执行确认/报告恢复，无硬编码中文。
...
```
v1.1.1.1 → … → v1.1.1.11 → v1.1.1.12 → v1.1.1.13
```
...
**工程版本**：开发期 `package.json` 随附属版递增；**v1.1.1** tag 已 push；后续 patch 可至 **1.1.1.11+**。
...
| patch | 主题 | 状态 |
|-------|------|:----:|
| **1.1.1.12** | 创建/复制计划后提示同步 `.aide` | ✅ |
| **1.1.1.13** | PanelHost 确认框 i18n | ✅ |

---

## 计划系统段落收官

**1.1.1.x 主线**已于 **1.1.1.13** 完成；下一步：**暂不 push** 或 Go **v1.1.2** 大更新。
