...
| **双重付费** | BYOK + 订阅 = 用户已在 API 平台充值，再收 IDE 费不合理 |

**旧战略**（[PLAN_IDE5_AND_COMPETITORS.md](./PLAN_IDE5_AND_COMPETITORS.md)）：不追 Kiro Spec、不追 Cursor Tab++。  
**新战略**：**主动冲击** Tab++/Spec 体验档，**平台 Key 多模型**（对标 Cursor 订阅含 AI），并用 **AIDE Runtime** 统一编排。
...
| AI 与编辑融合 | ~3.0 | **≥3.8**（Tab++ + **平台多模型**） |
| Agent / 多文件 | ~3.4 | **≥3.6**（Runtime + Activity Line） |
| 商业化 / 订阅 | ~3.0 | **≥3.3**（**弃 BYOK · 套餐含 AI**） |
...
| 支柱 | 对标 | 我们的做法 |
|------|------|------------|
| **平台 AI（F0）** | Cursor 订阅含 GPT/Claude | **平台 Key** · 6+ 厂商 · **无 BYOK** · 订阅覆盖成本 |
| **Tab++** | Cursor Copilot++ | 多行 FIM + 周围文件上下文 + P95&lt;400ms |
