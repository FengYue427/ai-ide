...
> **复评版本**：**v1.3.9**（2026-06-05）— [COMPETITOR_SCORE_V1.3.9.md](./COMPETITOR_SCORE_V1.3.9.md)  
> 执行跟踪与 [OPTIMIZATION_PLAN.md](./OPTIMIZATION_PLAN.md) 并行。

**评分说明**：0 = 无 · 1 = 演示 · 2 = 可用 · 3 = 生产级 · 4 = 行业领先  
**目标分**：日常主力 IDE 需多数维度 ≥ 3；**宣传线 ≥ 3.4**（v1.4 GA 目标）。

---

## 总览（v1.3.9 → 目标）

| 维度 | v1.3.9 | 目标 | 差距 | v1.4 承接 |
|------|:------:|:----:|------|-----------|
| 编辑器核心 | 3.2 | 3.5 | 小 | 抛光 |
| 语言服务 / 导航 | 3.0 | 3.5 | 中 | 非全 LSP，维持启发式 |
| 代码库理解 / 检索 | 3.2 | 3.5 | 小 | **F2** 2k Worker |
| AI 与编辑融合 | **3.0** | 3.5 | 中 | **F1** Tab P95 |
| Agent / 自动化 | **3.2** | 3.5 | 小 | **F5** 后台 Agent |
| 运行 / 调试 / Git | 2.6 | 3.0 | 中 | **F3** Git 面板 · **F4** 桌面 |
| 扩展 / MCP | 2.5 | 3.0 | 中 | **F6** 插件/MCP |
| 协作 / 团队 | 1.6 | 2.5 | 大 | Beta 维持 |
| 商业化 / 部署 | 2.8 | 3.0 | 小 | smoke 周更 |
| **综合（主力 IDE）** | **~3.28～3.32** | **~3.4** | **仍 &lt; 宣传线** | [V1.4_KICKOFF.md](./V1.4_KICKOFF.md) |
...
| ID | 能力 | Cursor/Kiro | 我们现状 | 目标 | 状态 |
|----|------|-------------|----------|------|------|
| C1 | Tab 级补全 | Copilot++ | FIM 链 + 指标可观测 | P95 &lt;800ms 生产默认 | 🔶 → **v1.4 F1** |
...
| C6 | 后台 Agent | 有 | 设置卡只读 | 生产 API + cron | 🔶 → **v1.4 F5** |
